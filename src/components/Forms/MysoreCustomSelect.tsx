...
interface MysoreCustomSelectProps {
  options: string[];
...
export const MysoreCustomSelect: React.FC<MysoreCustomSelectProps> = ({
  options,
  onChange,
  ...
  const errorMessage = {
    hu: 'Kérlek válassz tapasztalati szintet!',
    en: 'Please select experience level!',
  };
  ...
  return (
    <div ref={componentRef} className={`custom-select ${isInvalid ? 'custom-select--invalid' : ''}`}>
      ...
      {isInvalid && <div className="custom-select__error clr-error mt-8px">{errorMessage[language]}</div>}
    </div>
  );
};
